(->
  GuidService=->
    ...
    @getGuid=-> (S4()+S4()+"-"+S4()+"-"+S4()+"-"+S4()+"-"+S4()+S4()+S4());
    return @  

  RandomDateService=->
    @getRandomDate=(start=new Date(2011, 0, 1), end=new Date()) -> new Date(start.getTime() + Math.random() * (end.getTime() - start.getTime()));
    return @
  ...
  DatesDataService=->
    @getDatesData=(member,months)->
      ...
      return {"working":countWorking,"notWorking":countNotWorking}
    return @ 
  ...
  angular
    ...
    .service("RandomDateService",RandomDateService)
    .service("MembersService",MembersService)
    .service("DatesDataService",DatesDataService)
)()